...
class MLP:
    ...
    def softmax_function(self, vector_content):
        rt = np.exp(vector_content - np.max(vector_content)) / np.sum(np.exp(vector_content - np.max(vector_content)), axis=0)
        #rt = np.exp(vector_content - np.max(vector_content)) / (np.sum(np.exp(vector_content - np.max(vector_content)), axis=0) + 1e-30) # add beacuse randomly appears divisin by zero
        #print(rt)
        return rt
